...
var_df = var_df %>%
  ...
  mutate(across(any_of(vars_list), lag,.names = "l_{.col}")) %>% 
...
\itemsep3em
	...
	might have a significant impact on economic activity
	...
	\begin{itemize}
	  ...
	  \item
	  Bank credit growth and financial stability
	\end{itemize}
...
var_irf =  var_df  %>%
  dplyr::select(epu_log, fx_eff_log, i_il_log, emp_il_sa_log, gdp_il_sa_log) %>%
  dplyr::rename_all(~str_remove(.,"_log")) %>% 
  ts(start = c(1995, 2), frequency = 4)  %>%
  VAR(p = 8, ic = "AIC", type = "both", lag.max = 8)  %>%
  irf(n.ahead = 6,runs = 500,ci = 0.90,impulse = "epu")
